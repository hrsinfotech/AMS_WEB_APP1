...
function AttendancePage({ notify }: { notify: Notify }) {
  const rows = [
    { rule: "Standard Day Shift", start: "09:00", grace: "10 min", half: "4 hrs", full: "8 hrs", overtime: "After 8.5 hrs", night: "No" },
    { rule: "Night Shift", start: "22:00", grace: "15 min", half: "4 hrs", full: "8 hrs", overtime: "After 8 hrs", night: "Yes" },
  ];
  return <ModuleTablePage notify={notify} module="Attendance Configuration" title="Attendance Management" eyebrow="HRS TECH VIEW · MODULE 5" description="Shift-based attendance rule definition, rule-group assignment by day/holiday, and leave policy configuration." action="New Rule" tabs={["Attendance Rule", "Rule Group", "Leave Policy"]} columns={[{ key: "rule", label: "RULE NAME" }, { key: "start", label: "DAY START" }, { key: "grace", label: "GRACE PERIOD" }, { key: "half", label: "HALF-DAY MIN" }, { key: "full", label: "FULL-DAY MIN" }, { key: "overtime", label: "OVERTIME" }, { key: "night", label: "NIGHT SHIFT" }]} rows={rows} searchPlaceholder="Search attendance rules..." />;
}

function VisitorsPage({ notify }: { notify: Notify }) {
  const rows = [
    { visitor: "Renee Fontaine", company: "Meridian Consulting", host: "Ananya Rao", type: "Business", watchlist: "Clear", status: "Checked In" },
    { visitor: "Tomás Álvarez", company: "—", host: "Marta Kowalski", type: "Interview", watchlist: "Clear", status: "Expected" },
    { visitor: "Unknown — flagged", company: "—", host: "—", type: "Walk-in", watchlist: "Flagged", status: "Denied" },
    { visitor: "Grace Lin", company: "ACME MEP", host: "Daniel Osei", type: "Contractor Escort", watchlist: "Clear", status: "Overstaying" },
  ];
  return <ModuleTablePage notify={notify} module="Visitors" title="Visitor Management" eyebrow="HRS TECH VIEW · MODULE 6" description="End-to-end visitor lifecycle: gate/meeting-room setup, pre-registration policy, check-in/out, and watchlist screening." action="Pre-Register Visitor" stats={[{ label: "INSIDE NOW", value: "1", detail: "active visitor", tone: "good" }, { label: "EXPECTED TODAY", value: "1", detail: "pre-registered" }, { label: "OVERSTAYING", value: "1", detail: "Past approved window", tone: "warning" }, { label: "WATCHLIST FLAGS", value: "1", detail: "Requires justification to override", tone: "warning" }]} tabs={["Visitor Details", "Visit Details", "Gate Master", "Visitor Policy"]} columns={[{ key: "visitor", label: "VISITOR" }, { key: "company", label: "COMPANY" }, { key: "host", label: "HOST" }, { key: "type", label: "TYPE" }, { key: "watchlist", label: "WATCHLIST" }, { key: "status", label: "STATUS" }]} rows={rows} searchPlaceholder="Search visitor name or host..." />;
}
...
function AlarmsPage({ notify }: { notify: Notify }) {
  ...
  return <main className="mx-auto max-w-[1420px] px-4 pb-10 pt-[78px] sm:px-6 lg:px-8"><SectionHeading eyebrow="SECURITY VIEW · MODULE 15" title="Alarm Management" description="Classification, standard messaging, and policy assignment for security alarms." actions={<Button onClick={() => notify("New alarm class opened.", "info")} icon={Plus} kind="primary" testId="button-new-alarm">New</Button>} /><section className="control-surface overflow-hidden rounded-[7px]"><div className="border-b border-slate-800/80 px-3.5 py-3"><h2 className="text-[12px] font-semibold text-slate-200">Alarm Configuration</h2></div><div className="flex gap-4 border-b border-slate-800/80 bg-[#0f1b2b] px-3.5 pt-2.5">{["Alarm Class", "Escalation", "Instructions & Remarks", "Alarm Policy"].map((tab, index) => <button key={tab} onClick={() => notify(`${tab} configuration selected.`, "info")} className={`border-b-2 pb-2 text-[10px] ${index === 0 ? "border-cyan-400 font-semibold text-cyan-300" : "border-transparent text-slate-500"}`}>{tab}</button>)}</div><div className="grid gap-2.5 p-3.5 md:grid-cols-2 xl:grid-cols-4">{classes.map(([label, detail, color]) => <button key={label} onClick={() => notify(`${label} alarm class selected.`, label === "Critical" ? "warning" : "info")} className="rounded-[7px] border border-slate-800 bg-[#0e1928] p-3 text-left hover:border-cyan-500/40"><div className="flex items-center gap-2 text-[10px] font-semibold text-slate-200"><span className={`h-2 w-2 rounded-sm ${color}`} />{label}</div><div className="mt-2 text-[9px] text-slate-600">{detail}</div></button>)}</div></section></main>;
}
    ...
    <div className="dashboard-shell hairline-grid min-h-[100dvh] bg-[#09111d]">
      ...
      <div className="min-h-[100dvh] md:pl-[232px]">
        <Switch>
          ...
          <Route path="/reports"><ReportsPage notify={notify} /></Route>
          <Route path="/requests"><RequestsPage notify={notify} /></Route>
          <Route path="/analytics"><AnalyticsPage /></Route>
          <Route path="/settings"><SettingsPage notify={notify} /></Route>
